...
scan_factors = c('Duration','age18under','age55plus','sex','chronic_lung_disease',
                 ...
                 'surveillance','CT','median_coverage','run')
...
plot_feature_n = function(p_sub_scaled) {
  ...
  feature_n_plot = feature_stats %>% ggplot(aes(x=IP, y = OP, label = feature)) + 
    ...
    scale_y_continuous(breaks = seq(0,1500,100), limits = c(0,1300),
                       sec.axis = sec_axis(~ . / sum_n, labels = scales::label_percent(),
                                           breaks = seq(0,1,0.05))) + 
    scale_x_continuous(breaks = seq(0,2000,100), guide = guide_axis(angle = 90),
                       sec.axis = sec_axis(~ . / sum_n, labels = scales::label_percent(),
                                           breaks = seq(0,1,0.05))) + theme_pubr() +
    xlab(paste0("Inpatient (n & % of ", sum_n, ")")) +
    ylab(paste0("Outpatient (n & % of ", sum_n, ")"))
...
feature_n_plot = ggarrange(undated_feature_n_plot + ggtitle("12/2020 - 11/2021"), 
          ...
          labels = list("A","B"))
feature_n_plot
ggsave("ggsave/feature_n_plot.pdf", feature_n_plot, width = 14, height = 8)
```
...
x<-data.matrix(p_sub_scaled[, colnames(p_sub_scaled) %in% scan_factors])

p_sub_over = scale_scan_factors(
  patient_var_tmp %>% filter(n_var > 30 & CT > 26), 
                                scan_factors)
y_over<-log2(p_sub_over$n_var+1)
x_over<-data.matrix(p_sub_over[, colnames(p_sub_over) %in% scan_factors])
...
clinical_factors = c('sex','chronic_lung_disease', 'chronic_liver_disease', 'chronic_kidney_disease', 'chronic_heart_disease', 'transplant_patient', 'hiv', 'hypertension', 'diabetes', 'cancer', 'obesity', 'plasma', 'mAb','vaccine_status', 'surveillance_sample')
...
df = data.frame(); for (idx in 1:length(clinical_factors)) {
  ...
  vda = 1-vda(formula, tmp)
  pval = wilcox.test(formula, tmp)$p.value
  ...
  name_median = median_store[2,3][[1]] %>% as.numeric()
  ref_median = median_store[2,2][[1]] %>% as.numeric()
  name_n = 
  tmp_df = data.frame(name, name_median, ref_median, vda, pval, row.names = clinical_factors[idx])
...
sig_plot = df %>% filter(pval < alpha) %>% 
  ggplot(aes(vda,-log10(pval), label = name)) + geom_point() + 
  geom_vline(xintercept = 0.5, linetype = "dashed", color = "red") +
  geom_hline(yintercept = -log10(alpha), linetype = "dashed", color = "red") + 
  theme_pubr() + xlim(0.4, 0.6) + ylim(0,30) + 
  geom_text_repel(ylim = c(-log10(alpha),Inf), angle = 90)
...
plot_admitted = patient_counts_30 %>% group_by(admitted_hospital) %>% 
  ...
  geom_point(shape = "") +
        geom_density_2d(aes(color = admitted_hospital)) + stat_cor(method = "spearman", cor.coef.name = "rho") +
...
plot_vaccinated = patient_counts_30 %>%
  ...
  ggtitle
((
  plot_vaccinated_marginal_by_ct = ggMarginal(plot_vaccinated, type = "violin", 
                                            draw_quantiles = c(0,0.25,0.5, 0.75),
             groupColour = TRUE, groupFill = TRUE)
))
```
...
```{r}
# HIV GROUPS WITH CONTROL OVER THE TIME & OUTPATIENT STATUS
table(patient_counts_30 %>%  select(admitted_hospital,transplant_patient))

plot_hcw= patient_counts_30 %>% filter(admitted_hospital == 0) %>% filter(collection_date>="2021-07-01") %>%
  group_by(transplant_patient) %>% 
  ggplot(aes(x = INSTRUMENT_RESULT, y = n_var, color = transplant_patient)) + 
  geom_point(shape = "") +
        geom_density_2d(aes(color = transplant_patient)) + 
  stat_cor(method = "spearman", cor.coef.name = "rho") +
    geom_smooth(se=TRUE) + theme_pubr() + scale_y_continuous(trans="log1p", 
                                                             breaks = c(0,1,2,5,10,15,20,25,30))
((
