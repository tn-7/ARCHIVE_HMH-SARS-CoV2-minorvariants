...
unique_gene_counts = patient_var_30 %>% unite(col = "spectra_mutation", 
  ...
  select(gene, major, minor, spectra_mutation) %>% unique() %>% group_by(gene) %>% 
...
unique_all_enrichment_plot = unique_all %>% ggplot(aes(y = prop, x = gene, color = mutation)) + 
  ...
  theme_minimal() + coord_flip() + geom_hline(yintercept = 0, linetype = "dashed") +
  ylab("log2FC (observed / expected_counts)")

#ggsave("ggsave/unique_all_enrichment_plot.pdf", unique_all_enrichment_plot, height = 4, width = 4)
