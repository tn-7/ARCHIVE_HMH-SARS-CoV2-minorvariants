...
highly_shared_sites_ranked = patient_var_30 %>% group_by(ntpos) %>% tally() %>% 
  drop_na(ntpos) %>% arrange(-n) %>% mutate(rank = 1:nrow(.)) %>% 
  left_join(patient_var_30 %>% select(ntpos, gene, ref_sym, aapos, ref_codon, codon_pos) %>% distinct) %>%
  mutate(label = paste0(gene, ": ",ref_sym, aapos), refnt = str_sub(ref_codon, start = codon_pos, end = codon_pos))
...
highly_shared_sites_ranked %>% filter(rank < 100) %>% ggplot(aes(rank, n)) + geom_line(color = "grey") +
  scale_y_continuous(trans = "log2", breaks = 2^seq(0,12,1),
                     sec.axis = sec_axis(~./sample_n,
                                                          labels = scales::label_percent(),
                                           breaks = 2^seq(0,8,1)/100)) + theme_pubr() +
  geom_point(data = highly_shared_sites_ranked %>% filter(rank < 100) %>% filter(gene == "S"), aes(rank, n)) +
  geom_hline(yintercept = sample_n*0.01, linetype = "dashed", color = "salmon") +
  geom_vline(xintercept = 35, linetype = "dashed", color = "salmon") +
  
  geom_text_repel(data = highly_shared_sites_ranked %>% filter(rank < 100) %>% filter(gene == "S"), 
                  aes(label = label), ylim = c(6, Inf), angle = 90,
...
((
  hss_plot = patient_var_30 %>% filter(ntpos %in% highly_shared_sites) %>% 
    ...
    left_join(highly_shared_sites_ranked, by = "ntpos") %>% mutate(change=paste0(major,">",minor)) %>% 
    ...
    ggplot(aes(x=change, y=n.x, color=ref_reversion)) + 
...
```{r}
patient_var_30 %>% filter(ntpos == 24942)
```
...
consensus_hss<-consensus_sites_lowct %>% filter(ntpos %in% highly_shared_sites_ranked$ntpos[1:35]) %>% 
  mutate(MCoVNumber=mcov_reformat(name)) %>% left_join(runs_all_samples)

consensus_hss = fread("all_samples_hss.csv", data.table = F)
