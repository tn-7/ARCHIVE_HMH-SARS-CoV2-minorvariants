...
scale_and_run_binom = function(patient_counts_30, scan_factors_trim) {
  ...
  mutate(high_counts_threshold = 10, #quantile(.$n_var, probs = .8)[[1]], 
...
saveRDS(all_df_binom_nb, "ggsave/temp_all_df_binom_nb.rds")
```
...
all_df_binom_nb = readRDS("ggsave/temp_all_df_binom_nb.rds")
# order the y axis

df_input = all_df_binom_nb %>% filter(Source == "all_data")
forest_tn = function(df_input, model_count = 2) {
  ...
  forest_binom = df_input  %>%
    ...
    geom_effect(aes(xmin = (LL), xmax = (UL), 
    ...
    geom_stripes() + #annotation_logticks(sides = "tbl") +
    geom_vline(xintercept = 0, linetype = "dashed") + 
    scale_x_continuous(sec.axis = sec_axis(~ exp(.), breaks = c(seq(0.4,3,0.2)))) + #,
                       #labels=label_number(0.01)) + #geom_vline(xintercept = 1) +
    xlab("exp(Coefficient)")
  return(forest_binom)
}

forest_tn(all_df_binom_nb %>% filter(Source == "afterJuly_IP"))
forest_tn(all_df_binom_nb %>% filter(Source == "beforeJuly_IP"))
forest_tn(all_df_binom_nb %>% filter(Source == "afterJuly_OP"))
forest_tn(all_df_binom_nb %>% filter(Source == "beforeJuly_OP"))
forest_tn(all_df_binom_nb %>% filter(Source == "afterJuly"))
forest_tn(all_df_binom_nb %>% filter(Source == "beforeJuly"))
...
all_df_binom_nb %>% filter(Source == "afterJuly_IP")




tmp 
# patient_counts_30 %>% filter(collection_date >= "2021-07-01") %>% 
#   summarize(quant = quantile(.$n_var, probs = 0.8))
